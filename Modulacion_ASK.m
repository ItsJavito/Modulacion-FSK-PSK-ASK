...
%XXXXXXXXXXXXXXXXXXXXXXX Modulación ASK-binaria XXXXXXXXXXXXXXXXXXXXXXXXXXX%
...
ylabel('amplitud(volt)');
grid on;
title('Forma de onda para la modulación ASK correspondiente a los Datos binarios');
%XXXXXXXXXXXXXXXXXXXX Demodulación ASK-binaria XXXXXXXXXXXXXXXXXXXXXXXXXXXXX
mn=[];
...
ylabel('amplitud(volt)');
xlabel(' tiempo(seg)');
title('Datos binarios recibidos como señal digital después de la demodulación ASK');
%>>>>>>>>>>>>>>>>>>>>>>>>>> fin del programa >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>%
